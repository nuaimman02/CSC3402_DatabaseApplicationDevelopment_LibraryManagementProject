...
public interface BookRepository extends JpaRepository<Book,Long> {
    ...
    @Query(nativeQuery = true,value = "select b.id,b.isbn,b.name,b.description,b.quantity from book b inner join book_category c on b.id = c.book_id where c.category_id = :category and lower(b.name) like :name")
    Page<Book> findBooksByCategoryId(@Param("category") Long category,@Param("name") String name, Pageable pageable);

    @Query(nativeQuery = true,value = "select b.id,b.isbn,b.name,b.description,b.quantity from book b inner join book_author au on b.id = au.book_id where au.author_id = :author and lower(b.name) like :name")
